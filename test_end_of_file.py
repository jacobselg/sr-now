...
def test_end_of_file_restart():
    """Test MP4 restart functionality by testing near the end of file."""
    
    test_url = "https://lyssna-cdn.sr.se/Autorec/ET2W/P4/Sportextra/2025/10/SRP4RIKS_2025-10-13_190000_15300_a96.m4a"
    file_duration = 16530  # Duration in seconds (from our previous check)
    
    print(f"🧪 Testing MP4 end-of-file restart functionality...")
    print(f"📁 Test URL: {test_url}")
    print(f"⏱️ File duration: {file_duration} seconds")
    
    # Test positions near the end of the file
    test_positions = [
        file_duration - 60,  # 1 minute before end
        file_duration - 30,  # 30 seconds before end
        file_duration - 10,  # 10 seconds before end
        file_duration,       # Exactly at the end
        file_duration + 30,  # Past the end
        file_duration + 60   # Well past the end
    ]
    
    recording_length = 30
    
    for position in test_positions:
        print(f"\n--- Testing position {position}s (end - {file_duration - position}s) ---")
        
        chunk_path = get_audio_chunk_from_file(test_url, recording_length, position)
        
        if chunk_path is None:
            print(f"🔄 Reached end of file at position {position}s, simulating restart")
            # Test restart from beginning
            print("🔄 Testing restart from beginning...")
            restart_chunk = get_audio_chunk_from_file(test_url, recording_length, 0)
            if restart_chunk:
                print(f"✅ Successfully restarted from beginning")
                os.unlink(restart_chunk)
            else:
                print("❌ Failed to restart from beginning")
        else:
            print(f"✅ Successfully processed chunk from position {position}s")
            os.unlink(chunk_path)
        
        time.sleep(1)
    
    print("\n🎉 End-of-file restart test completed!")

if __name__ == "__main__":
    test_end_of_file_restart()